...
int main(){
    ...
    Point p[n];
    double c[n];
    for(int i = 0; i < n; i++)  cin >> p[i].x >> p[i].y >> c[i];
    auto judge = [&](Point a, double mid)->bool{
        int cnt = 0;
        for(int k = 0; k < n; k++)  cnt += getDistance(a, p[k]) <= mid/c[k]+EPS;
        return cnt >= X;
    };
    double l = 0, r = 1e9;
    for(int i = 0; i < 100; i++){
        ...
        for(int j = 0; j < n; j++){
            ok |= judge(p[j], mid);
            for(int k = j+1; k < n; k++){
                Circle A(p[j], mid/c[j]), B(p[k], mid/c[k]);
                if(!intersect(A, B))  continue;
                pair<Point,Point> P = getCrossPoints(A, B);
                ok |= judge(P.first, mid);
                ok |= judge(P.second, mid);
            }
        }
        if(ok)  r = mid;
        else    l = mid;
