...
int main(){
    ...
    double ans = getDistance(s, t);
    priority_queue<pair<double, int>> pq;
    for(int i = 0; i < n; i++){
        if(getDistance(s, v[i].c) <= v[i].r)    dp[i] = 0; 
        else                                    dp[i] = getDistance(s, v[i].c)-v[i].r;
        pq.push({-dp[i], i});
    }
    while(!pq.empty()){
        auto p = pq.top();  pq.pop();
        int i = p.second;
        if(dp[i] != -p.first)   continue;
        for(int j = 0; j < n; j++){
            if(i == j)  continue;
            double cost = 0;
            if(getDistance(v[i].c, v[j].c) > abs(v[i].r-v[j].r) && !intersect(v[i], v[j])){
                cost = getDistance(v[i].c, v[j].c) - v[i].r - v[j].r;
            }
            if(dp[i] + cost < dp[j]){
                dp[j] = dp[i] + cost;
                pq.push({-dp[j], j});
    ...
    for(int i = 0; i < n; i++){
        ans = min(ans, dp[i] + max(0.0, getDistance(v[i].c, t)-v[i].r));
    }
